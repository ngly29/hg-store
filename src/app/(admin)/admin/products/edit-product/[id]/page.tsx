...
export default function EditProduct() {
    ...
    const fetchProduct = async () => {
        ...
        try {
            ...
            const nextGallery = [...Array(4).fill("")];
            for (let i = 0; i < images.length; i += 1) {
                nextGallery[i] = images[i].imageData ?? "";
            }
    ...
    const handleUpdateProduct = async () => {
        try {
            ...
            if (coverFile) {
                await productApi.uploadProductImage(productId, coverFile, true);
            }

            if (galleryFiles.length > 0) {
                await productApi.uploadProductImages(productId, galleryFiles);
            }
            ...
            addNotification(
                "error",
                error?.response?.data?.message || error?.message || "Cập nhật sản phẩm thất bại!"
            );
        } finally {
